...
const STATUS_CONFIG = {
  ...
  [ApplicationStatus.CONFIRMATION_RECEIVED]: {
    name: 'Confirmed',
    color: '#16a34a',
    order: 1,
  }, // Darker green
  ...
  [ApplicationStatus.OFFER_NEGOTIATING]: {
    name: 'Negotiating',
    color: '#059669',
    order: 7.5,
  }, // Teal green
  ...
  [ApplicationStatus.POSITION_FILLED]: {
...
function removeCycles(
  ...
  links.forEach((link) => {
    ...
    if (sourceDepth < targetDepth) {
      ...
      console.log(
        `Removing circular/backward link: ${link.source} (depth: ${sourceDepth}) -> ${link.target} (depth: ${targetDepth})`,
      );
    }
  });
...
export function transformApplicationFlowToSankeyData(
  flowData: ApplicationFlowData[],
): SankeyData {
  ...
  console.log(
    'Sankey nodes:',
    nodes.map((n) => `${n.id} (depth: ${n.depth})`),
  );
  console.log(
    'Sankey links:',
  ...
  console.log(
    `Removed ${initialLinks.length - links.length} circular/invalid links`,
  );
...
export function getStatusColor(status: string, opacity: number = 1): string {
  const baseColor =
    STATUS_CONFIG[status as ApplicationStatus]?.color || '#6b7280';
