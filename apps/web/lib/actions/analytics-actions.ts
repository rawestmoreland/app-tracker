...
export async function getApplicationConversionRates(days?: number): Promise<{
  ...
  try {
    ...
    const transitions = await ActivityTracker.getUserStatusTransitions(
      dbUser.id,
      days || 90,
    );
    ...
    const conversionData = Array.from(stageAnalysis.entries())
      .map(([stage, analysis]) => ({
        stage,
...
export async function getHiringSuccessStats(days?: number): Promise<{
  ...
  try {
    ...
    const transitionStats = await ActivityTracker.getStatusTransitionStats(
      dbUser.id,
      days || 90,
    );
    ...
    const hiringRate =
      initialApplications > 0 ? (totalHired / initialApplications) * 100 : 0;
    ...
    const negativeStatuses: ApplicationStatus[] = [
      'REJECTED',
      'WITHDRAWN',
      'GHOSTED',
      'POSITION_FILLED',
